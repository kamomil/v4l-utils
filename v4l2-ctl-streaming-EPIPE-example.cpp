...
/*
   run with 
   v4l2-ctl -d1 --set-selection target=compose,width=1920,height=1080 -x width=1920,height=1080 -v width=1920,height=1080,pixelformat=YU12 --stream-mmap --stream-out-mmap --stream-from jelly_1920-1080-YU12.fwht --stream-to out-1920-1080.YU12 --stream-loop
...
static void read_write_padded_frame(cv4l_fmt &fmt, cv4l_queue &q, unsigned char* buf, FILE *fpointer, unsigned &sz, unsigned &len, bool is_read)
{
...
//	printf("padding: dafna: cropped: %ux%u, coded = %ux%u, composed: %ux%u\n", cropped_width, cropped_height, coded_width, coded_height, composed_width, composed_height);
	sz = 0;
...
//		printf("plane %d, div: %ux%u, step: %u\n",plane_idx, w_div, h_div, step);
...
restart:
	if (from_with_hdr) {
		...
		if (!fread(&v, sizeof(v), 1, fin)) {
			if (first) {
				fprintf(stderr, "fill buffer from file Insufficient data\n");
				return false;
			}
			if (stream_loop) {
	...
	for (unsigned j = 0; j < q.g_num_planes(); j++) {
		...
		if(support_out_crop && v4l2_fwht_find_pixfmt(fmt.g_pixelformat()))
			read_write_padded_frame(fmt, q, (unsigned char*) buf, fin, sz, len, true);
		else {
			printf("fill_buffer_from_file: len = %u\n",len);
			sz = fread(buf, 1, len, fin);
		}

		//getchar();
		if (first && sz != len) {
			fprintf(stderr, "fill buffer from file Insufficient data %u %u\n",sz,len);
			return false;
		}
...
static void write_buffer_to_file(cv4l_fd &fd, cv4l_queue &q, cv4l_buffer &buf, FILE *fout)
{
	...
	for (unsigned j = 0; j < buf.g_num_planes(); j++) {
		...
		if (host_fd_to >= 0) {
			...
			read_write_padded_frame(fmt, q, (u8 *)q.g_dataptr(buf.g_index(), j) + offset, fout, sz, used, false);
		}
		else {
			 ...
			 unsigned &count, fps_timestamps &fps_ts)
{
	...
	//printf("do_handle_cap\n");
	for (;;) {
		ret = fd.dqbuf(buf);
		if (ret == EAGAIN) {
			printf("do_handle_cap 11\n");
			return 0;
		}
		if (ret == EPIPE) {
			printf("do_handle_cap 22\n");
			return -2;
		}
		if (ret) {
...
static void streaming_set_m2m(cv4l_fd &fd)
{
	...
	printf("%s: about to enter while\n",__func__);
	while (rd_fds || wr_fds || ex_fds) {
		struct timeval tv = { 2, 0 };
		int r = 0;

		printf("%s: starting while\n",__func__);
		...
		if (rd_fds && FD_ISSET(fd.g_fd(), rd_fds)) {
			...
			if (r < 0) {
				printf("%s: do handle cap returned %d\n",__func__, r);
				sleep(2);
				//rd_fds = NULL;
				if (!have_eos) {
					ex_fds = NULL;
			...
			if (r < 0)  {
				...
				if (have_eos) {
					...
					if (is_encoder)
						...
						printf("%s: calling decoder cmd stop\n", __func__);
						fd.decoder_cmd(dec_stop);
					}
				} else {
					printf("%s: file finished breaking\n", __func__);
					break;
				}
			}
			//sleep(5);
